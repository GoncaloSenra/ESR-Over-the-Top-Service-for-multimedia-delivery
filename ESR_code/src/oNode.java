...
public class oNode {
    ...
    public ConcurrentHashMap<String, ConcurrentLinkedQueue<InetAddress>> bestPath; // lista de caminhos para o cliente
                                                                                   // de cada conteudo

    public ConcurrentHashMap<String, ConcurrentLinkedQueue<InetAddress>> bestPathInv; // lista de caminhos para o RP de
                                                                                      // cada conteudo
    ...
    private ConcurrentHashMap<String, InetAddress> streams_IP; // lista de streams ativas no node com o ip do servidor
                                                               // que esta a streamar
    ...
    DatagramSocket searchResultSocket;

    DatagramSocket pongSocket;

    DatagramSocket pingSocket;

    DatagramSocket pingCSocket;

    DatagramSocket cancelSocket;

    DatagramSocket cancelSocketClient;

    DatagramSocket helloSocket;

    DatagramSocket searchSocket;

    DatagramSocket BestPathSocket;
    ...
    public oNode() { // construtor
        ...
        try {
            ...
            this.searchResultSocket = new DatagramSocket(6001);

            this.pongSocket = new DatagramSocket(8500);

            this.pingSocket = new DatagramSocket(5000);
            this.pingSocket.setSoTimeout(1000);

            this.pingCSocket = new DatagramSocket(2000);
            this.pingCSocket.setSoTimeout(1000);

            this.cancelSocket = new DatagramSocket(6666);

            this.cancelSocketClient = new DatagramSocket(6500);

            this.helloSocket = new DatagramSocket(5003);

            this.searchSocket = new DatagramSocket(6000);

            this.BestPathSocket = new DatagramSocket(7000);
    ...
    private void StreamServer() { // stream desde os servers ate ao cliente
        ...
        while (true) {
            try {
                ...
                if (rtp_packet.getsequencenumber() == 1) {
                    System.out.println("===STREAM recieved (send to client)===");
                    System.out.println("RECEIVED: " + rtp_packet.getVideoName() + " from " + receivePacket.getAddress()
                            + ":" + 9000);
                }
    ...
    private void checkVideo() {
        ...
        while (true) {
            try {
                Thread.sleep(25000); // vai verificar se as metricas sao as melhores
                // System.out.println("checkVideo pre semaphore");
                semaphore.acquire();
                System.out.println("===CHECK (if has video)===");
                ...
                for (String stream : streams) {
                    ...
                    for (ServerInfo entry : servers) {
                        ...
                        for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry2 : bestPath
                                .entrySet()) {
                    ...
                    if (!streams_IP.get(stream).getHostAddress().equals(ip.getHostAddress())) {
                        System.out.println("[Feedback]: Nao tem video requisitado");
                ...
                System.err.println("[Feedback]: Erro na thread checkVideo");
            }
        }
                ...
                for (ServerInfo entry : servers) {
                    for (int i = 0; i < 3; i++) {
                        ...
                        try {
                            ...
                            // entry.getKey().getAddress().getHostName() + " -----");
                            ...
                            if (i == 2) {
                                ...
                                new Thread(() -> {
                                    try {
                                        ...
                                        for (ConcurrentHashMap.Entry<String, InetAddress> entry2 : streams_IP
                                                .entrySet()) {
                                            if (entry2.getValue().equals(entry.getAddress())) {
                                                streams_IP.remove(entry2.getKey());

                                                // se não houver outro server com a mesma stream então remove da lista
                                                // de streams
                                                ...
                                                if (ip != null) {
                                                    ...
                                                    byte[] datak = baos.toByteArray();
                                                    DatagramPacket sendPacket = new DatagramPacket(datak, datak.length,
                                                            ip, 9999);
                                                    pingSocket.send(sendPacket);
                                                    ...
                                                    for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry3 : bestPath
                                                            .entrySet()) {
                                                        if (entry3.getKey().trim().equals(entry2.getKey().trim())) {
                                                            for (InetAddress ip3 : entry3.getValue()) {
                                                                DatagramPacket sendPacket = new DatagramPacket(datak,
                                                                        datak.length, ip3, 6500);
                                                                pingSocket.send(sendPacket);
                                                                System.out.println("SENT CANCEL -> to: "
                                                                        + ip3.getHostAddress() + ":" + 6500);
                                                            }
                ...
                for (InetAddress entry : ip_clients) {
                    for (int i = 0; i < 3; i++) {
                        ...
                        try {
                            ...
                            // System.out.println("PING enviado para: " +
                            // entry.getKey().getAddress().getHostName());
                                ...
                                for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry2 : bestPath
                                    ...
                                    if (entry2.getValue().contains(entry)) {
                                        ...
                                        if (lista.size() == 1) {
                                            ...
                                            for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry3 : bestPathInv
                                                    .entrySet()) {
                                                if (entry3.getKey().trim().equals(entry2.getKey().trim())) {
                                                    for (InetAddress ipInv : entry3.getValue()) {
                                                        DatagramPacket sendPacket1 = new DatagramPacket(data,
                                                                data.length, ipInv, 6666);
                                                        pingCSocket.send(sendPacket1);
                                                        System.out.println("SENT: " + entry2.getKey() + " to " + ipInv
                                                                + ":" + 6666);
                                                    }
    ...
    private void cancelStream() { // cancelar o envio de uma stream para um cliente de um router ate ao RP/biforcacao
        ...
        while (true) {
            try {
                ...
                System.out.println("===CANCEL stream to Server===");
                ...
                for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry2 : bestPath.entrySet()) {
                    // System.out.println("entry2: " +"|"+ entry2.getKey()+ "|" + "entry" + "|" +
                    // entry + "|");
                            ...
                            if (RP) {
                                for (ConcurrentHashMap.Entry<String, InetAddress> entry3 : streams_IP.entrySet()) {
                                    if (entry3.getKey().trim().equals(entry.trim())) {
                                        ...
                                        oos2.close();
                                        byte[] datak2 = baos2.toByteArray();
                                        DatagramPacket sendPacket2 = new DatagramPacket(datak2, datak2.length,
                                                entry3.getValue(), 9999);// envia para tras na porta 9999
                                        cancelSocket.send(sendPacket2);
                                        System.out.println("SENT to server -> to: " + entry3.getValue().getHostAddress()
                                                + ":" + 9999);
                            ...
                            for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry3 : bestPathInv
                                ...
                                if (entry3.getKey().trim().equals(entry2.getKey().trim())) {
                                    for (InetAddress ipInv : entry3.getValue()) {
                                        ...
                                        System.out.println(
                                                "SENT to next: " + entry2.getKey() + " to " + ipInv + ":" + 6666);
                ...
                System.err.println("[Feedback]: Erro na thread cancelStream");
            }
        }
    ...
    private void cancelStreamClient() { // router ate cliente a avisar que vai deixar de receber a stream "X"
        ...
        while (true) {
            try {
                ...
                System.out.println("=== CANCEL stream to Client ===");
                ...
                for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry2 : bestPath.entrySet()) {
                    ...
                    if (entry2.getKey().trim().equals(entry.trim())) {
                        ...
                        if (RP) {
                            for (ConcurrentHashMap.Entry<String, InetAddress> entry3 : streams_IP.entrySet()) {
                                if (entry3.getKey().trim().equals(entry.trim())) {
                                    ...
                                    System.out.println(
                                            "SENT to server-> to: " + entry3.getValue().getHostAddress() + ":" + 9999);
                                    streams_IP.remove(entry3.getKey());
                        ...
                        for (InetAddress ipInv : entry2.getValue()) {
                            ...
                            System.out.println("SENT to client: " + entry2.getKey() + " to " + ipInv + ":" + 6500);
                        }
                ...
                System.err.println("[Feedback]:Erro na thread cancelStreamClient");
            }
        }
                ...
                if (!exists) {
                    try {
                        ...
                        if (readObject instanceof Packet) {
                            ...
                            System.out.println("===Connect Server(Add new)===");
                            System.out.println("RECEIVED (server): " + servers.toString() + " from "
                                    + receivePacket.getAddress() + ":" + 5003);
                        }
            ...
            try {
                ...
                if (readObject instanceof Packet) {
                    ...
                    if (streams.contains(str) || RP) { // router tem a stream -> vai enviar para tras para a origem de acordo com o path no pacote
                        ...
                        if (flag) {
                            System.out.println("[Feedback]: Contem stream");
                            p.setHops(1);
                        ...
                        System.out.println("[Feedback]: Nao contem stream");
                        ByteArrayOutputStream baos = new ByteArrayOutputStream();
                        ObjectOutputStream oos = new ObjectOutputStream(baos);
                        oos.writeObject(p);
                        oos.close();
                        ...
                        System.out.println("[Feedback]: not RP and not stream");

                        for (ConcurrentHashMap.Entry<IpWithMask, Boolean> entry : activeRouters.entrySet()) {
                            System.out.println("Routers vizinhos: " + entry.getKey().getNetwork().getHostAddress());
                            ...
                            if (entry.getValue()) { // se o router estiver ativo
                                ...
                                if (!sent) {
                                    System.out.println("[Feedback]: Router ativo(" + entry.getKey().getAddress() + ") e sem envios");
                                    DatagramPacket sendPacket = new DatagramPacket(datak, datak.length,
                                            entry.getKey().getAddress(), 6000);
                                    searchSocket.send(sendPacket);
                                    System.out.println("SENT to router: " + str + " to " + entry.getKey().getAddress()+ ":" + 6000);
                                } else {
                                    System.out.println("[Feedback]: Pacote ja enviado para este router");
                                }
                            }
                        }
                    }
        ...
        while (true) {
            ...
            try {
                ...
                if (readObject instanceof Packet) {
                    ...
                    System.out.println("===UPDATE BestPath===");
                    ...
                    if (!streams.contains(str)) {
                        ...
                        if (RP) {
                            System.out.println("[Feedback]: Sou RP!");
                            InetAddress ip = null;
                            double latency = 1000000000;
                            ...
                            System.out.println(
                                    "[Feedback]: IP escolhido (metricas): " + ip.getHostAddress() + " -> " + latency);
                            ...
                            System.out.println("SENT to server -> to: " + ip.getHostAddress() + ":" + 9999);
                            streams_IP.put(str, ip);
                        } else {
                            // System.out.println("[Feedback]: Not RP!");
                            p.setHops(p.getHops() + 1);

                            // FIXME: Debug caso aconteca aquele erro
                        ...
                        System.out.println("[Feedback]: Tem stream");
                        if (!bestPath.containsKey(str)) {
                            ConcurrentLinkedQueue<InetAddress> IPAdd = new ConcurrentLinkedQueue<InetAddress>();
                            IPAdd.add(IPAddress);
                            bestPath.put(str, IPAdd);
        ...
        while (true) {
            try {
                ...
                if (readObject instanceof Packet) {
                    ...
                    // + ":" + 6001 + " -> " + p.getPathInv().toString() + " -> " + p.getHops() +
                    // p.getPath().toString());
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        try {
                            ...
                            if (addr.getHostName().equals(entry.getKey().getAddress().getHostName())) {
                                // System.out.println("PONG recebido de: " +
                                // entry.getKey().getAddress().getHostName());
                                entry.setValue(true);
                            ...
                            if (i == 2) {
                                if (entry.getValue()) {
                                            ...
                                            if (entry2.getValue().size() == 1) {// mandar tudo down ate ao rp
                                                ...
                                                for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry3 : bestPathInv
                                                    ...
                                                    if (entry3.getKey().trim().equals(entry2.getKey().trim())) {
                                                        data = entry2.getKey().getBytes();
                                                        DatagramPacket sendPacket1 = new DatagramPacket(data,
                                                                data.length, entry3.getValue().peek(), 6666);
                                                        pingSocket.send(sendPacket1);
                                                        System.out.println("SENT: " + entry2.getKey() + " to "
                                                                + entry3.getValue().peek() + ":" + 6666);
                                                ...
                                                if (RP) {
                                                    for (ConcurrentHashMap.Entry<String, InetAddress> entry3 : streams_IP
                                                            .entrySet()) {
                                                        if (entry3.getKey().trim().equals(entry2.getKey().trim())) {
                                                            semaphore.acquire();
                                                            ...
                                                            oos2.close();
                                                            byte[] datak2 = baos2.toByteArray();
                                                            DatagramPacket sendPacket2 = new DatagramPacket(datak2,
                                                                    datak2.length, entry3.getValue(), 9999);// envia
                                                                                                            // para tras
                                                                                                            // na porta
                                                                                                            // 9999
                                                            pingSocket.send(sendPacket2);
                                                            System.out.println("SENT to server-> to: "
                                                                    + entry3.getValue().getHostAddress() + ":" + 9999);
                                                            streams_IP.remove(entry3.getKey());
                                                            streams.remove(entry2.getKey().trim());
                                                            semaphore.release();
                                                            break;
                                    ...
                                    for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry4 : bestPathInv
                                            .entrySet()) {

                                        if (entry4.getValue().contains(entry.getKey().getAddress())) {
                                            bestPathInv.remove(entry4.getKey());

                                            for (ConcurrentHashMap.Entry<String, ConcurrentLinkedQueue<InetAddress>> entry5 : bestPath
                                                    .entrySet()) {
                                                if (entry5.getKey().trim().equals(entry4.getKey().trim())) {
                                                    data = entry5.getKey().getBytes();
                                                    for (InetAddress ips : entry5.getValue()) {
                                                        DatagramPacket sendPacket1 = new DatagramPacket(data,
                                                                data.length, ips, 6500);
                                                        pingSocket.send(sendPacket1);
                                                        System.out.println(
                                                                "SENT: " + entry5.getKey() + " to " + ips + ":" + 6500);
                                                    }
                                                            ...
                                                            if (entry3.getKey().trim().equals(entry4.getKey().trim())) {
                                                                ...
                                                                DatagramPacket sendPacket2 = new DatagramPacket(datak2,
                                                                        datak2.length, entry3.getValue(), 9999);// envia
                                                                                                                ...
                                                                                                                // 9999
                                                                pingSocket.send(sendPacket2);
                                                                System.out.println("SENT to s -> to: "
                                                                        + entry3.getValue().getHostAddress() + ":"
                                                                        + 9999);
    ...
    private void pong() {
        ...
        while (true) {
            try {
                ...
                boolean exists = false;
                for (ConcurrentHashMap.Entry<IpWithMask, Boolean> entry : activeRouters.entrySet()) {
                    if (entry.getKey().getAddress().getHostAddress()
                ...
                if (!exists) {
                    ...
                    try {
                        ...
                        if (readObject instanceof InetAddress) {
                            ...
                            System.out.println("[Feedback]: Adicionado: " + ip.getAddress().getHostAddress());
