...
class Server {
    ...
    public void bestPath(){ //vai procurar o melhor caminho para o destino
        //1 envia pacote search(5000)
        //2 espera pelos pacotes a receber(5001)
        //3 escolhe o melhor caminho
        //4 envia o pacote para o melhor caminho sendo esse pacote noutra porta (5005)
        
        //variaveis
        ConcurrentLinkedQueue<Packet> pacotes = new ConcurrentLinkedQueue<Packet>();
        
        try {
            DatagramSocket searchSocket = new DatagramSocket(5001);
            //1
            ...
            DatagramPacket sendPacket = new DatagramPacket(data, data.length, ip_router.getAddress(), 5000);
            searchSocket.send(sendPacket);
            System.out.println("SENT: " + "search" + " to " + ip_router.getAddress() + ":" + 5000);
            ...
            DatagramPacket sendPacket2 = new DatagramPacket(datak, datak.length, dest2, 5005);//envia para tras na porta 6001
            sendSocket.send(sendPacket2);
            System.out.println("SENT to pc -> to: " + dest2.getHostAddress() + ":" + 5005);
